...
export const WhatsAppSimulator: React.FC<WhatsAppSimulatorProps> = ({
  ...
  const handleSendChoice = (option: string) => {
    ...
    if (option === '1') {
      ...
      newMsgs.push({
        sender: 'bot',
        text: `📞 Your assigned LGA Field Agent is ${selectedBen?.assigned_agent_name || 'LGA Officer'}. We have sent them a notification to reach out to ${selectedBen?.phone_number || 'your phone'}.`,
        time,
      });
    } else {
      newMsgs.push({
  ...
  return (
    <div className="max-w-lg mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl space-y-5">
      ...
      <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
            <Bot className="w-5 h-5 text-emerald-400" /> WhatsApp Cloud API Bot Simulator
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Lightweight self-checkin channel for digitally capable beneficiaries.
          </p>
        </div>
        <span className="bg-emerald-950 text-emerald-400 border border-emerald-800 px-2 py-0.5 rounded text-[11px] font-bold">
          Meta WhatsApp API
        </span>
      </div>

      {/* Select Beneficiary Context */}
      <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl space-y-1 text-xs">
        <label className="block text-slate-400 font-semibold">Simulate Beneficiary Phone Session:</label>
        <select
          value={selectedBenId}
          onChange={(e) => setSelectedBenId(e.target.value)}
          className="w-full bg-slate-900 border border-slate-700 text-slate-100 text-xs rounded p-2 focus:outline-none font-semibold"
        >
          {beneficiaries.map((b) => (
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 h-80 overflow-y-auto space-y-3 font-sans">
        {messages.map((m, idx) => (
          <div
            key={idx}
          ...
          >
            <div
              className={`p-3 rounded-2xl text-xs leading-relaxed shadow ${
                m.sender === 'user'
                  ? 'bg-emerald-700 text-white rounded-br-none font-bold'
      ...
      {/* Interactive Number Reply Buttons */}
      <div className="space-y-2">
        <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
          Quick Simulated WhatsApp Replies:
        </div>

        <div className="grid grid-cols-3 gap-2">
          <button
            onClick={() => handleSendChoice('1')}
            className="bg-emerald-950 hover:bg-emerald-900 text-emerald-300 border border-emerald-700 p-2.5 rounded-lg text-xs font-bold transition"
          >
            Reply 1 (Running)
          </button>
          <button
            onClick={() => handleSendChoice('2')}
            className="bg-red-950 hover:bg-red-900 text-red-300 border border-red-700 p-2.5 rounded-lg text-xs font-bold transition"
          >
            Reply 2 (Stopped)
          </button>
          <button
            onClick={() => handleSendChoice('3')}
            className="bg-amber-950 hover:bg-amber-900 text-amber-300 border border-amber-700 p-2.5 rounded-lg text-xs font-bold transition"
          >
            Reply 3 (Agent)
          </button>
        </div>
